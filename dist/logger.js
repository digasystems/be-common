...
exports.mainLogger = (0, winston_1.createLogger)({
    format: winston_1.format.combine(winston_1.format.errors({ stack: true }), winston_1.format.timestamp({ format: 'DD-MM-YYYY HH:mm:ss' }), winston_1.format.align(), winston_1.format.printf(info => `[${info.level}] ${[info.timestamp]}: ${info.message}`)),
    transports: [
